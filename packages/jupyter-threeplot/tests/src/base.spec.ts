...
describe('BaseModel', () => {
  ...
  describe('getObjectFromCache()', () => {
    ...
    it('should return undefined for undefined cache descriptors', () => {
      expect(BaseModel.getObjectFromCache(undefined)).to.be(undefined);
      expect(BaseModel.getObjectFromCache({id: undefined!})).to.be(undefined);
      expect(BaseModel.getObjectFromCache({id: null!})).to.be(undefined);
      expect(BaseModel.getObjectFromCache({id: 'not a valid ID'})).to.be(undefined);
      expect(BaseModel.getObjectFromCache({} as any)).to.be(undefined);
    ...
    it('should get an existing model', () => {
      let model = createTestModel(TestModel);
      return model.initPromise.then(() => {
        let cache = BaseModel.getObjectFromCache(model.getCacheDescriptor());
